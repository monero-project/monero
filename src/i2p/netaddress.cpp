...
void CNetAddr::SetLegacyIPv6(Span<const uint8_t> ipv6) {
  ...
  if (HasPrefix(ipv6, IPV4_IN_IPV6_PREFIX)) {
    ...
    // TORv2-in-IPv6 (unsupported). Unserialize as !IsValid(), thus ignoring
    // them. Mimic a default-constructed CNetAddr object which is !IsValid() and
    // thus will not be gossiped, but continue reading next addresses from the
    // stream.
    m_net = NET_IPV6;
...
bool CNetAddr::IsRFC6052() const {
  return IsIPv6() && HasPrefix(m_addr, std::array<uint8_t, 12>{
...
bool CNetAddr::IsRFC4862() const {
  return IsIPv6() &&
         HasPrefix(m_addr, std::array<uint8_t, 8>{0xFE, 0x80, 0x00, 0x00, 0x00,
                                                  0x00, 0x00, 0x00});
}
...
bool CNetAddr::IsRoutable() const {
  return IsValid() &&
         !(IsRFC1918() || IsRFC2544() || IsRFC3927() || IsRFC4862() ||
           IsRFC6598() || IsRFC5737() || IsRFC4193() || IsRFC4843() ||
 ...
 */
bool CNetAddr::IsInternal() const { return m_net == NET_INTERNAL; }
...
// Return an IPv6 address text representation with zero compression as described
// in RFC 5952
// ("A Recommendation for IPv6 Address Text Representation").
static std::string IPv6ToString(Span<const uint8_t> a, uint32_t scope_id) {
  assert(a.size() == ADDR_IPV6_SIZE);
  const std::array groups{
      ReadBE16(&a[0]), ReadBE16(&a[2]),  ReadBE16(&a[4]),  ReadBE16(&a[6]),
      ReadBE16(&a[8]), ReadBE16(&a[10]), ReadBE16(&a[12]), ReadBE16(&a[14]),
  };
  ...
  r.reserve(39);
  for (size_t i{0}; i < groups.size(); ++i) {
    // Replace the longest sequence of consecutive all-zero fields with two
    // colons ("::").
...
uint32_t CNetAddr::GetLinkedIPv4() const {
  if (IsIPv4()) {
    ...
    // mapped IPv4, SIIT translated IPv4: the IPv4 address is the last 4 bytes
    // of the address
