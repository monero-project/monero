...
echo "Generating:"
doxygen Doxyfile && echo "Backup previous version:" && rm -rf ~/triton-www-previous && mv "$wwwdir/doc" ~/triton-www-previous && cp -ar doc/ "$wwwdir/" && echo "Done, builded and copied to public - the doxygen docs" && echo "size:" && du -Dsh "$wwwdir/" && echo "files:" && find "$wwwdir/" | wc -l
